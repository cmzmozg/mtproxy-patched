...
| `995` | POP3S | Обычно не трогают |
| `465` | SMTPS | Обычно не трогают |
| `5222` | XMPP | Обычно не трогают |
| `8883` | MQTT over TLS | Обычно не трогают |
| `2443` | Нестандартный | **Шейпят** на мобильных операторах, задержка x10-x30 |
| `8443` | Нестандартный | **Шейпят**, задержка до 5 секунд |

Порт `853` — лучший выбор. Если он занят — `993`, `995`, `465`. Порт `443` обычно занят xray/VLESS ([xray-setup](https://github.com/cmzmozg/xray-setup)). Портов `2443`, `8443` и подобных — избегать.
