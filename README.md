...
sudo bash install.sh 853 vkvideo.ru
...
Telegram-клиент
    |
    |  FakeTLS (провайдер видит HTTPS к vkvideo.ru)
    v
...
```
MT_TLS_DOMAIN=vkvideo.ru        # дефолт
...
**Рекомендуемые порты:** `853` (DNS-over-TLS) или `443` (HTTPS). Провайдеры и ТСПУ не шейпят трафик на этих портах, в отличие от нестандартных (2443, 8443 и т.д.).
...
| `MT_TLS_DOMAIN` | `vkvideo.ru` | Домен маскировки |
